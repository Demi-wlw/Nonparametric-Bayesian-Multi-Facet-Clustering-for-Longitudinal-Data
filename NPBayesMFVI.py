...
class MFNLG(MFVI):
    def __init__(self, inter_knots, times=None, degree: int=1, Bscale: bool=True, **kwargs):
        """Bayesian Nonparametric Multi-Facet Nonlinear growth model with Mean-Field Variational Inference
        ...
        """
        print("######################################################")
        print("### Mean-Field Variational Inference for BNPMF-NLG ###")
        print("######################################################")
        ...
        self.B = np.asarray(B).T # I x T matrix
    ...
    def __compute_ELBO(self, est_logV_a, est_log1V_a, est_logV_beta, est_log1V_beta, est_logV_tau, est_log1V_tau):
        ...
        ELBO = 0
        # prior -q on a
        ELBO += sum([1/2*(np.log(self.__lambda_a2)-np.log(self.__lamStar_a[k,1])) +1/2 - 1/2*self.__lambda_a2* (1/self.__lamStar_a[k,1]+self.__est_a[k]**2-2*self.__lambda_a1*self.__est_a[k]+self.__lambda_a1**2) for k in range(self.K)])
        # prior -q on beta
        ELBO += sum([self.__I/2*(np.log(self.__lambda_beta2))-1/2*np.array(LA.slogdet(self.__lamStar_beta2[k])).prod() +self.__I/2 - 1/2*self.__lambda_beta2* (np.sum(self.__est_beta[k,:]**2) + np.trace(LA.pinv(self.__lamStar_beta2[k])) -2*self.__lambda_beta1*np.sum(self.__est_beta[k,:]) +self.__I*self.__lambda_beta1**2) for k in range(self.K)])
        # prior -q on tau
        ELBO += sum([self.__lambda_tau1*np.log(self.__lambda_tau2) - self.__lamStar_tau[k,0]*np.log(self.__lamStar_tau[k,1]) -gammaln(self.__lambda_tau1)+gammaln(self.__lamStar_tau[k,0]) +(self.__lambda_tau1-self.__lamStar_tau[k,0])*(psi(self.__lamStar_tau[k,0])-np.log(self.__lamStar_tau[k,1])) +self.__lamStar_tau[k,0] -self.__lambda_tau2*self.__est_tau[k] for k in range(self.K)])
        # prior -q on v
        ELBO += sum([(self.__alphfix-self.__alphaStar_a[k,0])*est_logV_a[k] + (self.__est_alpha_a-self.__alphaStar_a[k,1])*est_log1V_a[k] - psi(self.__sStar_a[0]) +np.log(self.__sStar_a[1]) +betaln(self.__alphaStar_a[k,0],self.__alphaStar_a[k,1]) for k in range(self.K-1)])
        ELBO += sum([(self.__alphfix-self.__alphaStar_beta[k,0])*est_logV_beta[k] + (self.__est_alpha_beta-self.__alphaStar_beta[k,1])*est_log1V_beta[k] - psi(self.__sStar_beta[0]) +np.log(self.__sStar_beta[1]) +betaln(self.__alphaStar_beta[k,0],self.__alphaStar_beta[k,1]) for k in range(self.K-1)])
        ELBO += sum([(self.__alphfix-self.__alphaStar_tau[k,0])*est_logV_tau[k] + (self.__est_alpha_tau-self.__alphaStar_tau[k,1])*est_log1V_tau[k] - psi(self.__sStar_tau[0]) +np.log(self.__sStar_tau[1]) +betaln(self.__alphaStar_tau[k,0],self.__alphaStar_tau[k,1]) for k in range(self.K-1)])
        for n in range(self.N):
            # for Z_a_n - max piStar_a[n, :]
            ELBO += sum([np.sum(self.__piStar_a[n, (k+1):self.K])*est_log1V_a[k] +self.__piStar_a[n,k]*est_logV_a[k] for k in range(self.K-1)]) - (np.log(self.__piStar_a[n, self.__est_Za[n]]) if self.__piStar_a[n, self.__est_Za[n]] >0 else 0)
            # for Z_beta_n
            ELBO += sum([np.sum(self.__piStar_beta[n, (k+1):self.K])*est_log1V_beta[k] +self.__piStar_beta[n,k]*est_logV_beta[k] for k in range(self.K-1)]) - (np.log(self.__piStar_beta[n, self.__est_Zbeta[n]]) if self.__piStar_beta[n, self.__est_Zbeta[n]] >0 else 0)
            # for Z_tau_n
            ELBO += sum([np.sum(self.__piStar_tau[n, (k+1):self.K])*est_log1V_tau[k] +self.__piStar_tau[n,k]*est_logV_tau[k] for k in range(self.K-1)]) - (np.log(self.__piStar_tau[n, self.__est_Ztau[n]]) if self.__piStar_tau[n, self.__est_Ztau[n]] >0 else 0)
            # likelihood for y_n
            Yobs, Bobs, Tobs = self.__get_obs_data(n)
            E_a_n = sum([self.__piStar_a[n,j]* self.__est_a[j] for j in range(self.K)])
            E_beta_n = sum([self.__piStar_beta[n,j]* self.__est_beta[j, :] for j in range(self.K)])
            E_tau_n = sum([self.__piStar_tau[n, j]*self.__est_tau[j] for j in range(self.K)])
            E_log_tau_n = sum([self.__piStar_tau[n, j]*(psi(self.__lamStar_tau[j,0]) - np.log(self.__lamStar_tau[j,1])) for j in range(self.K)])
            E_a2_n = sum([self.__piStar_a[n,j]* (1/self.__lamStar_a[j,1]+self.__est_a[j]**2) for j in range(self.K)])
    ...
    def fit(self, maxIter: int=200, prune_threshold: float=0.01, ELBO_rel_tol: float=1e-8, ELBO_abs_tol: float=1e-4, verbose: bool=False):
        ...
        for it in tqdm(range(maxIter), desc="[INFO] Iterate the Coordinate Ascent algorithm"): # loop over iterations for MFVI
            ...
            est_Va = np.zeros(self.K-1)
            est_Vbeta = np.zeros(self.K-1)
            est_Vtau = np.zeros(self.K-1)
            est_logV_a = np.zeros(self.K-1)
            est_log1V_a = np.zeros(self.K-1)
            est_logV_beta = np.zeros(self.K-1)
            est_log1V_beta = np.zeros(self.K-1)
            est_logV_tau = np.zeros(self.K-1)
            est_log1V_tau = np.zeros(self.K-1)
            ...
            for k in (tqdm(range(self.K), desc='[INFO] Update variational parameters for all Ks',leave=False) if verbose else range(self.K)): 
                if k < self.K-1:
                    ...
                    self.__alphaStar_a[k, 1] = self.__est_alpha_a + np.sum(self.__piStar_a[:, (k+1):self.K])
                    self.__alphaStar_beta[k, 0] = self.__alphfix + np.sum(self.__piStar_beta[:, k])
                    self.__alphaStar_beta[k, 1] = self.__est_alpha_beta + np.sum(self.__piStar_beta[:, (k+1):self.K])
                    self.__alphaStar_tau[k, 0] = self.__alphfix + np.sum(self.__piStar_tau[:, k])
                    self.__alphaStar_tau[k, 1] = self.__est_alpha_tau + np.sum(self.__piStar_tau[:, (k+1):self.K])
                    #--- get means for V ---#
            ...
            self.__est_pi_a[self.K-1] = 1 - self.__est_pi_a[:(self.K-1)].sum()
            self.__est_pi_beta[self.K-1] = 1 - self.__est_pi_beta[:(self.K-1)].sum()
            self.__est_pi_tau[self.K-1] = 1 - self.__est_pi_tau[:(self.K-1)].sum()

            ## Cluster reordering ##
            order_idx_a = np.flip(np.argsort(self.__est_pi_a[:self.K])) # sort by est. pi in descending order
            order_idx_a = np.concatenate((order_idx_a, np.arange(self.K, len(self.__est_pi_a)))) # add zeros to the end
            order_idx_beta = np.flip(np.argsort(self.__est_pi_beta[:self.K]))
            order_idx_beta = np.concatenate((order_idx_beta, np.arange(self.K, len(self.__est_pi_beta))))
            order_idx_tau = np.flip(np.argsort(self.__est_pi_tau[:self.K]))
            order_idx_tau = np.concatenate((order_idx_tau, np.arange(self.K, len(self.__est_pi_tau))))
            ...
            for n in (tqdm(range(self.N), desc='[INFO] Update variational parameters for Zn', leave=False) if verbose else range(self.N)): 
                E_a_n = sum([self.__piStar_a[n,j]* self.__est_a[j] for j in range(self.K)]) # can be rewritten as a matrix-vector multiplication?
                E_beta_n = sum([self.__piStar_beta[n,j]* self.__est_beta[j, :] for j in range(self.K)])
                E_tau_n = sum([self.__piStar_tau[n, j]* self.__est_tau[j] for j in range(self.K)])
                Yobs, Bobs, Tobs = self.__get_obs_data(n)
            ...
            self.__sStar_a[0] = self.__s1 + self.K -1
            self.__sStar_a[1] = self.__s2 - est_log1V_a.sum()
            self.__sStar_beta[0] = self.__s1 + self.K -1
            self.__sStar_beta[1] = self.__s2 - est_log1V_beta.sum()
            self.__sStar_tau[0] = self.__s1 + self.K -1
            ...
            pruned_idx_a = np.where(self.__est_pi_a[:self.K] < prune_threshold)[0] 
            pruned_idx_beta = np.where(self.__est_pi_beta[:self.K] < prune_threshold)[0]
            pruned_idx_tau = np.where(self.__est_pi_tau[:self.K] < prune_threshold)[0]
            ...
            self.__piStar_a[:,:self.K] = softmax(self.__piStar_a[:,:self.K], axis=1) # renormalize to sum to 1 (this function is stable)
            self.__piStar_beta[:,:self.K] = softmax(self.__piStar_beta[:,:self.K], axis=1) # (no floating point error)
            self.__piStar_tau[:,:self.K] = softmax(self.__piStar_tau[:,:self.K], axis=1)
            self.__est_Za = np.argmax(self.__piStar_a[:,:self.K], axis=1)
            self.__est_Zbeta = np.argmax(self.__piStar_beta[:,:self.K], axis=1)
            self.__est_Ztau = np.argmax(self.__piStar_tau[:,:self.K], axis=1)
            ...
            if self._has_converged(ELBO_rel_tol, ELBO_abs_tol):
                #* final check and pruning with fixed prune threshold
                pruned_idx_a = np.where(self.__est_pi_a[:self.K] > 0.01)[0]
                pruned_idx_beta = np.where(self.__est_pi_beta[:self.K] > 0.01)[0]
                pruned_idx_tau = np.where(self.__est_pi_tau[:self.K] > 0.01)[0]
                self.__est_Za = np.argmax(self.__piStar_a[:,pruned_idx_a], axis=1)
                self.__est_Zbeta = np.argmax(self.__piStar_beta[:,pruned_idx_beta], axis=1)
                self.__est_Ztau = np.argmax(self.__piStar_tau[:,pruned_idx_tau], axis=1)
                if verbose: 
    ...
    def plotFacet(self, estDict: dict=None, facet=['a', 'beta', 'sigma'], aTshift: tuple=(0,0), save_folder: str=None, figDict: dict=None):
        ...
        if facets[0] in facet:
            ...
            for i in range(len(est_a)):
                plt.plot(self.times+aTshift[1], [est_a[i]+aTshift[0]]*len(self.times), "--o", alpha=min(pi_a[i]*100,1), markevery=max(self.times)+1, label="a{}".format(i+1))
            plt.xticks(fontsize=fontsize)
            plt.yticks(fontsize=fontsize)
        ...
        if facets[1] in facet:
            ...
            for i in range(est_beta.shape[0]):
                plt.plot(self.times+aTshift[1], est_beta[i]@ self.B, alpha=min(pi_beta[i]*100,1), label="Beta{}".format(i+1))
            plt.xticks(fontsize=fontsize)
            plt.yticks(fontsize=fontsize)
            plt.xlabel('Time', fontsize=fontsize+2)
...
class MFVAR(MFVI):
    def __init__(self, **kwargs):
        """Bayesian Nonparametric Multi-Facet Vector autoregressive model with Mean-Field Variational Inference
        ...
        """
        print("######################################################")
        print("### Mean-Field Variational Inference for BNPMF-VAR ###")
        print("######################################################")
        super().__init__(**kwargs)
        #--- data distribution ---#
        ...
        self.__ColprecStar_Beta = np.zeros((self.K, self.D, self.D)) # column precision matrices (V)
    ...
    def __compute_ELBO(self, est_logV_a, est_log1V_a, est_logV_Beta, est_log1V_Beta, est_logV_tau, est_log1V_tau):
        ...
        ELBO = 0
        # prior -q on a
        ELBO += sum([self.D/2*np.log(self.__prior_epsa) -1/2*np.array(LA.slogdet(self.__LamStar_a[k,:,:])).prod() - self.__prior_epsa/2* (LA.norm(self.__est_a[k,:]-self.__prior_mua)**2) for k in range(self.K)]) #  + np.trace(LA.pinv(self.__LamStar_a[k,:,:]))
        # prior -q on Beta
        ELBO += sum([self.D**2/2*np.log(self.__prior_epsBeta) -1/2*np.array(LA.slogdet(self.__LamStar_Beta[k,:,:])).prod() - self.__prior_epsBeta/2* (LA.norm(self.__est_Beta[k]-self.__prior_muBeta)**2 + np.trace(LA.pinv(self.__LamStar_Beta[k]))) for k in range(self.K)])
        # prior -q on tau
        for k in range(self.K):
        ...
        for n in range(self.N):
            ...
            E_Beta_n = sum([self.__piStar_Beta[n,j]* self.__est_Beta[j, :,:] for j in range(self.K)])
            E_tau_n = sum([self.__piStar_tau[n, j]*self.__est_tau[j,:] for j in range(self.K)])
            sum_E_log_tau_n = sum([sum([self.__piStar_tau[n, j]*(psi(self.__lamStar_tau[j,d,0]) - np.log(self.__lamStar_tau[j,d,1])) for j in range(self.K)]) for d in range(self.D)])
            ELBO += 1/2*sum_E_log_tau_n - 1/2*(self.Y[n,:,0]-E_a_n).T@ np.diag(E_tau_n)@(self.Y[n,:,0]-E_a_n) + sum([1/2*sum_E_log_tau_n - 1/2*(self.Y[n,:,t]-E_a_n-E_Beta_n@self.Y[n,:,t-1]+E_Beta_n@E_a_n).T@np.diag(E_tau_n)@(self.Y[n,:,t]-E_a_n-E_Beta_n@self.Y[n,:,t-1]+E_Beta_n@E_a_n) for t in range(1,self.T)])
        # prior -q on alpha
        ELBO += self.__s1*np.log(self.__s2) - self.__sStar_a[0]*np.log(self.__sStar_a[1]) -gammaln(self.__s1)+gammaln(self.__sStar_a[0]) +(self.__s1-self.__sStar_a[0])*(psi(self.__sStar_a[0])-np.log(self.__sStar_a[1])) -self.__s2*self.__est_alpha_a +self.__sStar_a[0]
        ELBO += self.__s1*np.log(self.__s2) - self.__sStar_Beta[0]*np.log(self.__sStar_Beta[1]) -gammaln(self.__s1)+gammaln(self.__sStar_Beta[0]) +(self.__s1-self.__sStar_Beta[0])*(psi(self.__sStar_Beta[0])-np.log(self.__sStar_Beta[1])) -self.__s2*self.__est_alpha_Beta +self.__sStar_Beta[0]
        ELBO += self.__s1*np.log(self.__s2) - self.__sStar_tau[0]*np.log(self.__sStar_tau[1]) -gammaln(self.__s1)+gammaln(self.__sStar_tau[0]) +(self.__s1-self.__sStar_tau[0])*(psi(self.__sStar_tau[0])-np.log(self.__sStar_tau[1])) -self.__s2*self.__est_alpha_tau +self.__sStar_tau[0]
        ...
        for it in tqdm(range(maxIter), desc="[INFO] Iterate the Coordinate Ascent algorithm"): # loop over iterations for MFVI
            ...
            for k in (tqdm(range(self.K), desc='[INFO] Update variational parameters for all Ks',leave=False) if verbose else range(self.K)): 
                ...
                for n in range(self.N):
                    E_tau_n = sum([self.__piStar_tau[n, j]*self.__est_tau[j,:] for j in range(self.K)])
                    E_a_n = sum([self.__piStar_a[n,j]* self.__est_a[j,:] for j in range(self.K)])
                    E_a_mat_n = (E_a_n+np.zeros((self.D,self.T-1)).T).T
                    Betap1_sum_n = self.__piStar_Beta[n, k] * np.diag(E_tau_n)
                    BetaXX_n = self.Y[n,:,:-1]-E_a_mat_n
                    Beta2_sum += self.__piStar_Beta[n, k] * np.kron(BetaXX_n @ BetaXX_n.T, np.diag(E_tau_n))
                    Beta1_sum += Betap1_sum_n @ (self.Y[n,:,1:]-E_a_mat_n) @ BetaXX_n.T
                ...
                self.__ColprecStar_Beta[k,:,:] = np.diag(np.diag(self.__LamStar_Beta[k]).reshape(self.D, self.D, order='F').mean(axis=0))
                ...
                for n in range(self.N):
                    ...
                    a1_sum += self.T * a1_sum_n + (self.T-1)*a2_sum_n - (self.T-1)*a3_sum_n - (self.T-1)*a4_sum_n
                    a2_sum += a1_sum_n @ self.Y[n,:,0] + a1_sum_n @ np.sum(self.Y[n,:,1:], axis=1) - a4_sum_n @ np.sum(self.Y[n,:,:-1], axis=1) + a2_sum_n@np.sum(self.Y[n,:,:-1], axis=1) - a3_sum_n@np.sum(self.Y[n,:,1:], axis=1)
                ...
                for n in range(self.N):
                    E_a_n = sum([self.__piStar_a[n,j]* self.__est_a[j,:] for j in range(self.K)])
                    E_Beta_n = sum([self.__piStar_Beta[n,j]* self.__est_Beta[j,:,:] for j in range(self.K)])
                    tau2_sum_n = self.__piStar_tau[n, k] * ((self.Y[n,:,0]-E_a_n)**2 + sum([(self.Y[n,:,t]-E_a_n- E_Beta_n@self.Y[n,:,t-1]+E_Beta_n@E_a_n)**2 for t in range(1,self.T)]))
                    tau2_sum += tau2_sum_n
                self.__lamStar_tau[k,:,0] = np.zeros(self.D) + self.__lambda_tau1 + self.T/2 * np.sum(self.__piStar_tau[:, k])
                self.__lamStar_tau[k,:,1] = self.__lambda_tau2 + 1/2 * tau2_sum
                #--- get means for tau ---#
                self.__est_tau[k,:] = self.__lamStar_tau[k,:, 0]/self.__lamStar_tau[k,:, 1]
            # get pi for the last cluster
            ...
            ## Cluster reordering ##
            order_idx_a = np.flip(np.argsort(self.__est_pi_a[:self.K])) # sort by est. pi in descending order
            order_idx_a = np.concatenate((order_idx_a, np.arange(self.K, len(self.__est_pi_a))))
            order_idx_Beta = np.flip(np.argsort(self.__est_pi_Beta[:self.K]))
            order_idx_Beta = np.concatenate((order_idx_Beta, np.arange(self.K, len(self.__est_pi_Beta))))
            order_idx_tau = np.flip(np.argsort(self.__est_pi_tau[:self.K]))
            order_idx_tau = np.concatenate((order_idx_tau, np.arange(self.K, len(self.__est_pi_tau))))
            ...
            for n in (tqdm(range(self.N), desc='[INFO] Update variational parameters for Zn', leave=False) if verbose else range(self.N)): 
                ...
                for k in range(self.K):
                    # update variational parameters for a: PiStar_a
                    mat_a = (self.Y[n,:,1:].T-self.__est_a[k,:]).T - E_Beta_n@(self.Y[n,:,:-1].T-self.__est_a[k,:]).T
                    Sa_kn = -1/2 * ( (self.Y[n,:,0]-self.__est_a[k,:]).T @ np.diag(E_tau_n) @ (self.Y[n,:,0]-self.__est_a[k,:]) + np.trace(np.diag(E_tau_n)@mat_a@mat_a.T) ) 
                    self.__piStar_a[n, k] = est_logV_a[k:(k+1)].sum() + est_log1V_a[:k].sum() + Sa_kn # enable indexing out of bounds -> 0
                    # update variational parameters for beta: PiStar_Beta
                    mat_Beta = (self.Y[n,:,1:].T-E_a_n).T - self.__est_Beta[k,:,:]@(self.Y[n,:,:-1].T-E_a_n).T
                    Sb_kn = -1/2 * ( (self.Y[n,:,0]-E_a_n).T @ np.diag(E_tau_n) @ (self.Y[n,:,0]-E_a_n) + np.trace(np.diag(E_tau_n)@mat_Beta@mat_Beta.T) ) 
                    self.__piStar_Beta[n, k] = est_logV_Beta[k:(k+1)].sum() + est_log1V_Beta[:k].sum() + Sb_kn
                    # update variational parameters for tau: PiStar_tau
                    mat_tau = (self.Y[n,:,1:].T-E_a_n).T - E_Beta_n@(self.Y[n,:,:-1].T-E_a_n).T
                    Stau_kn = self.T/2* np.log(self.__est_tau[k,:]).sum() -1/2 * ( (self.Y[n,:,0]-E_a_n).T @ np.diag(self.__est_tau[k,:]) @ (self.Y[n,:,0]-E_a_n) + np.trace(np.diag(self.__est_tau[k,:])@mat_tau@mat_tau.T) ) 
                    self.__piStar_tau[n, k] = est_logV_tau[k:(k+1)].sum() + est_log1V_tau[:k].sum() + Stau_kn
